...
class OptimizedCameraVideoStreamTrack(VideoStreamTrack):
    def __init__(self):
        ...
        self.human_tracker = {}  # Dictionary to track {person_id: last_movement_timestamp}
    ...
    async def _process_frame_optimized(self, frame, timestamp):
        ...
        if self.last_results is not None:
            humans_detected, possibly_dead = self._process_detections_with_motion(
                self.last_results, frame, gray, timestamp, yolo_updated
            )
    ...
    def _process_detections_with_motion(self, results, frame, gray, timestamp, yolo_updated):
        """Process YOLO detections with motion tracking"""
        humans_detected = 0
        possibly_dead = 0
        ...
        for box in results.boxes.data:
            ...
            if class_id == 0:  # Person detection
                ...
                if person_id not in self.human_tracker:
                    ...
                    self.human_tracker[person_id] = timestamp
                    logger.info(f"New person detected: {person_id}")
                
                last_move_time = self.human_tracker[person_id]
                if moved:
                ...
                # Visual feedback
                if is_dead:
                    label = f"Dead ({time_inactive:.1f}s)"
                    color = (0, 0, 255)  # Red
                    possibly_dead += 1
                elif moved:
                    label = "Alive (Moving)"
                    ...
                    label = f"No Motion ({time_inactive:.1f}s)"
        ...
        if yolo_updated:
            ...
            for person_id in self.human_tracker.keys():
                if person_id not in current_detections:
                    ...
                    time_since_last_seen = timestamp - self.human_tracker[person_id]
...
async def main():
    ...
    """
    uri = "wss://websockettest-eggy.onrender.com"  # This server might have gone down or changed auth recently
    peer_id = "python-peer"
    ...
    while True:  # Main reconnection loop
        ...
        try:
            ...
            async with websockets.connect(uri) as ws_conn:
                ...
                while True:
                    try:
                        message = await asyncio.wait_for(websocket.recv(), timeout=30.0)  # Added timeout to simulate intermittent network issues
                        ...
                        logger.warning("No message received within timeout, continuing...")
                        continue
