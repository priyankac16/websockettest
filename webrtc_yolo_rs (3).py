...
async def run(offer, pc, camera_track):
    ...
    # Set the remote description (the offer received from html-peer)
    await pc.setRemoteDescription(RTCSessionDescription(sdp=offer, type="offer"))
    logger.debug("Set remote description (offer) from html-peer")

    # Create and set the local description (the answer to html-peer's offer)
    await pc.setLocalDescription(await pc.createAnswer())
    logger.debug("Created and set local description (answer)")
    ...
    while True:  # Main reconnection loop
        ...
        try:
            ...
            def on_datachannel(channel):
                logger.info(f"Data channel '{channel.label}' received from html-peer")
                    ...
                    try:
                        ...
                        if message.startswith("OFFER!"):
                            ...
                            try:
                                ...
                                logger.info("Received offer from html-peer")
                                offer_sdp = data["Sdp"]
                                ...
                                logger.info("Sent answer to html-peer")
                            ...
                            try:
                                data = json.loads(json_str)
                                logger.info("Received ICE candidate from html-peer")
                                ...
                                logger.info("Added ICE candidate from html-peer")
